...
sub OnInit{
    ...
    $self->{xresWin} = Wx::XmlResource->new();						#create the resource object
    Wx::XmlResource::AddSubclassFactory( 							#add the custom class to enable XRC subclassing
    	CPANPLUS::Shell::Wx::Frame::XRCFactory->new ); 
    $self->{xresWin}->InitAllHandlers();							#initialize all XRC resource handlers
    $self->{xresWin}->Load($xrc_file);								#load the file
    my $mainWin=CPANPLUS::Shell::Wx::Frame->new($self);				#create a new default frame
    $self->{xresWin}->LoadFrame( $mainWin,undef, 'main_window' )	#load the XRC-defined frame into the default frame
      or return;
    print _T("[Done]\n");
