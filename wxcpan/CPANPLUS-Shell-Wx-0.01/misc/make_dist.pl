...
#create the archive
#TODO create .tar.gz instead of .gz
use IO::Compress::Gzip qw(gzip $GzipError);
my $curdir=File::Spec->rel2abs(File::Spec->curdir);
my @split=File::Spec->splitdir($curdir);
my $filename = $split[-2];
my $status = gzip \@compress => $filename.'.gz'
            or die "gzip failed: $GzipError\n";

#my $tar = Archive::Tar->new;
#unless (Archive::Tar->create_archive($filename.'.gz',undef, @compress)){
#    print Archive::Tar->error;
#}
#$tar->add_files(@compress);
#$tar->write($filename.".tar",0);
